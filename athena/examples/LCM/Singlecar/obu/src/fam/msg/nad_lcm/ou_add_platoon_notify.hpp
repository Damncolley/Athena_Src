...
#ifndef __nad_lcm_ou_add_platoon_notify_hpp__
#define __nad_lcm_ou_add_platoon_notify_hpp__
...
/// obu->vui：创建编队通知（仅RSU创建成功时发出）
class ou_add_platoon_notify
{
    public:
...
int ou_add_platoon_notify::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int ou_add_platoon_notify::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int ou_add_platoon_notify::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t ou_add_platoon_notify::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* ou_add_platoon_notify::getTypeName()
{
    return "ou_add_platoon_notify";
}

int ou_add_platoon_notify::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int ou_add_platoon_notify::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
...
int ou_add_platoon_notify::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t ou_add_platoon_notify::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == ou_add_platoon_notify::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)ou_add_platoon_notify::getHash };

    uint64_t hash = 0x284ca8ed47ed8214LL +
