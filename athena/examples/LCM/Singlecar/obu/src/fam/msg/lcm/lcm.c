...
lcm_create (const char *url)
{
    ...
    int i1 = 0;
    for (; i1 < providers->len; i1++) {
        lcm_provider_info_t * pinfo = (lcm_provider_info_t *) g_ptr_array_index (providers, i1);
...
lcm_destroy (lcm_t * lcm)
{
    if (lcm->provider){
        unsigned int i2 = 0;
        for (; i2 < lcm->handlers_all->len; i2++) {
    ...
    unsigned int i3 = 0;
    for (; i3 < lcm->handlers_all->len; i3++) {
        lcm_subscription_t *h = (lcm_subscription_t *) g_ptr_array_index(
                lcm->handlers_all, i3);
...
lcm_get_handlers (lcm_t * lcm, const char * channel)
{
    ...
    unsigned int i4 = 0;
    for (; i4 < lcm->handlers_all->len; i4++) {
        lcm_subscription_t *h = (lcm_subscription_t *) g_ptr_array_index (lcm->handlers_all, i4);
...
lcm_try_enqueue_message(lcm_t* lcm, const char* channel)
{
    ...
    int num_keepers = 0;
    unsigned int i5 = 0;
    for(; i5<handlers->len; i5++) {
        lcm_subscription_t* h = (lcm_subscription_t*) g_ptr_array_index(handlers, i5);
...
lcm_dispatch_handlers (lcm_t * lcm, lcm_recv_buf_t * buf, const char *channel)
{
    ...
    int i6 = 0;
    for (; i6 < nhandlers; i6++) {
        lcm_subscription_t *h = (lcm_subscription_t *) g_ptr_array_index(handlers, i6);
    ...
    int i7 = 0;
    for (; i7 < nhandlers; i7++) {
        lcm_subscription_t *h = (lcm_subscription_t *) g_ptr_array_index(handlers, i7);
    ...
    int i8 = 0;
    for (; i8 < nhandlers; i8++) {
        lcm_subscription_t *h = (lcm_subscription_t *) g_ptr_array_index(handlers, i8);
        h->callback_scheduled = 0;
        ...
        GHashTable * args)
{
    ...
    *provider = strdup (provider_networkargs[0]);
    ...
    if (network_args[0] && network_args[1]) {
        char ** option_arg = network_args + 1;
        for (; *option_arg != NULL ;option_arg++) {
            // parse the args
            char ** split_args = g_strsplit(*option_arg, "&", -1);
            int i9 = 0;
            for (; split_args[i9]; i9++) {
                char ** key_value = g_strsplit(split_args[i9], "=", 2);
