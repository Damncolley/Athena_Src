...
#ifndef __nad_lcm_ro_add_platoon_notify_hpp__
#define __nad_lcm_ro_add_platoon_notify_hpp__
...
/// rsu->obu：创建编队通知（仅RSU创建成功时发出）
class ro_add_platoon_notify
{
    public:
...
int ro_add_platoon_notify::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int ro_add_platoon_notify::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int ro_add_platoon_notify::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t ro_add_platoon_notify::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* ro_add_platoon_notify::getTypeName()
{
    return "ro_add_platoon_notify";
}

int ro_add_platoon_notify::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int ro_add_platoon_notify::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
...
int ro_add_platoon_notify::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t ro_add_platoon_notify::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == ro_add_platoon_notify::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)ro_add_platoon_notify::getHash };
