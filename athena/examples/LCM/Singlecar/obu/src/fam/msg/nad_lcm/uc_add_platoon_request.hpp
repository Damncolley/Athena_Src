/** THIS IS AN AUTOMATICALLY GENERATED FILE.  DO NOT MODIFY
 * BY HAND!!
 *
 * Generated by lcm-gen
 **/

#include <lcm/lcm_coretypes.h>

#ifndef __nad_lcm_uc_add_platoon_request_hpp__
#define __nad_lcm_uc_add_platoon_request_hpp__

#include <string>
#include <vector>
#include "nad_lcm/key_point.hpp"

namespace nad_lcm
{

/**
 * -------------------------------------------------------------------
 * 编队消息：创建编队
 * 本期仅支持一个编队，platoon_name恒为"platoon"
 * -------------------------------------------------------------------
 * oct->csu：创建编队请求
 */
class uc_add_platoon_request
{
    public:
        std::string platoon_name;

        int16_t    num_of_platoon;

        std::vector< std::string > platoon;

        double     speed;

        nad_lcm::key_point key_point;

        std::string destination;

    public:
        /**
         * Encode a message into binary form.
         *
         * @param buf The output buffer.
         * @param offset Encoding starts at thie byte offset into @p buf.
         * @param maxlen Maximum number of bytes to write.  This should generally be
         *  equal to getEncodedSize().
         * @return The number of bytes encoded, or <0 on error.
         */
        inline int encode(void *buf, int offset, int maxlen) const;

        /**
         * Check how many bytes are required to encode this message.
         */
        inline int getEncodedSize() const;

        /**
         * Decode a message from binary form into this instance.
         *
         * @param buf The buffer containing the encoded message.
         * @param offset The byte offset into @p buf where the encoded message starts.
         * @param maxlen The maximum number of bytes to reqad while decoding.
         * @return The number of bytes decoded, or <0 if an error occured.
         */
        inline int decode(const void *buf, int offset, int maxlen);

        /**
         * Retrieve the 64-bit fingerprint identifying the structure of the message.
         * Note that the fingerprint is the same for all instances of the same
         * message type, and is a fingerprint on the message type definition, not on
         * the message contents.
         */
        inline static int64_t getHash();

        /**
         * Returns "uc_add_platoon_request"
         */
        inline static const char* getTypeName();

        // LCM support functions. Users should not call these
        inline int _encodeNoHash(void *buf, int offset, int maxlen) const;
        inline int _getEncodedSizeNoHash() const;
        inline int _decodeNoHash(const void *buf, int offset, int maxlen);
        inline static uint64_t _computeHash(const __lcm_hash_ptr *p);
};

int uc_add_platoon_request::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    int64_t hash = (int64_t)getHash();

    tlen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &hash, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = this->_encodeNoHash(buf, offset + pos, maxlen - pos);
    if (tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int uc_add_platoon_request::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;

    int64_t msg_hash;
    thislen = __int64_t_decode_array(buf, offset + pos, maxlen - pos, &msg_hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;
    if (msg_hash != getHash()) return -1;

    thislen = this->_decodeNoHash(buf, offset + pos, maxlen - pos);
    if (thislen < 0) return thislen; else pos += thislen;

    return pos;
}

int uc_add_platoon_request::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t uc_add_platoon_request::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* uc_add_platoon_request::getTypeName()
{
    return "uc_add_platoon_request";
}

int uc_add_platoon_request::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;

    char* platoon_name_cstr = (char*) this->platoon_name.c_str();
    tlen = __string_encode_array(buf, offset + pos, maxlen - pos, &platoon_name_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __int16_t_encode_array(buf, offset + pos, maxlen - pos, &this->num_of_platoon, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    for (int a0 = 0; a0 < this->num_of_platoon; a0++) {
        char* __cstr = (char*) this->platoon[a0].c_str();
        tlen = __string_encode_array(buf, offset + pos, maxlen - pos, &__cstr, 1);
        if(tlen < 0) return tlen; else pos += tlen;
    }

    tlen = __double_encode_array(buf, offset + pos, maxlen - pos, &this->speed, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = this->key_point._encodeNoHash(buf, offset + pos, maxlen - pos);
    if(tlen < 0) return tlen; else pos += tlen;

    char* destination_cstr = (char*) this->destination.c_str();
    tlen = __string_encode_array(buf, offset + pos, maxlen - pos, &destination_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int uc_add_platoon_request::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;

    int32_t __platoon_name_len__;
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &__platoon_name_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__platoon_name_len__ > maxlen - pos) return -1;
    this->platoon_name.assign(((const char*)buf) + offset + pos, __platoon_name_len__ - 1);
    pos += __platoon_name_len__;

    tlen = __int16_t_decode_array(buf, offset + pos, maxlen - pos, &this->num_of_platoon, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    this->platoon.resize(this->num_of_platoon);
    for (int a0 = 0; a0 < this->num_of_platoon; a0++) {
        int32_t __elem_len;
        tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &__elem_len, 1);
        if(tlen < 0) return tlen; else pos += tlen;
        if(__elem_len > maxlen - pos) return -1;
        this->platoon[a0].assign(((const char*)buf) + offset + pos, __elem_len -  1);
        pos += __elem_len;
    }

    tlen = __double_decode_array(buf, offset + pos, maxlen - pos, &this->speed, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = this->key_point._decodeNoHash(buf, offset + pos, maxlen - pos);
    if(tlen < 0) return tlen; else pos += tlen;

    int32_t __destination_len__;
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &__destination_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__destination_len__ > maxlen - pos) return -1;
    this->destination.assign(((const char*)buf) + offset + pos, __destination_len__ - 1);
    pos += __destination_len__;

    return pos;
}

int uc_add_platoon_request::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
    enc_size += this->platoon_name.size() + 4 + 1;
    enc_size += __int16_t_encoded_array_size(NULL, 1);
    for (int a0 = 0; a0 < this->num_of_platoon; a0++) {
        enc_size += this->platoon[a0].size() + 4 + 1;
    }
    enc_size += __double_encoded_array_size(NULL, 1);
    enc_size += this->key_point._getEncodedSizeNoHash();
    enc_size += this->destination.size() + 4 + 1;
    return enc_size;
}

uint64_t uc_add_platoon_request::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == uc_add_platoon_request::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)uc_add_platoon_request::getHash };

    uint64_t hash = 0xd68d4169cf710a28LL +
         nad_lcm::key_point::_computeHash(&cp);

    return (hash<<1) + ((hash>>63)&1);
}

}

#endif
