...
#ifndef __nad_lcm_uo_upcall_request_hpp__
#define __nad_lcm_uo_upcall_request_hpp__
...
/**
 * -------------------------------------------------------------------
 * 后门：允许VUI上报命令给OBU/RSU/CSU执行
 * -------------------------------------------------------------------
 * vui->obu：发起向上调用
 */
class uo_upcall_request
{
    public:
        std::string obu_name;
...
int uo_upcall_request::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int uo_upcall_request::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int uo_upcall_request::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t uo_upcall_request::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* uo_upcall_request::getTypeName()
{
    return "uo_upcall_request";
}

int uo_upcall_request::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    ...
    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->upcall_type, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    ...
    for (int a0 = 0; a0 < this->num_of_platoon; a0++) {
        char* __cstr = (char*) this->platoon[a0].c_str();
    ...
    tlen = this->key_point._encodeNoHash(buf, offset + pos, maxlen - pos);
    if(tlen < 0) return tlen; else pos += tlen;
...
int uo_upcall_request::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
    ...
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->upcall_type, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    ...
    for (int a0 = 0; a0 < this->num_of_platoon; a0++) {
        ...
        this->platoon[a0].assign(((const char*)buf) + offset + pos, __elem_len -  1);
        pos += __elem_len;
    }
    ...
    tlen = this->key_point._decodeNoHash(buf, offset + pos, maxlen - pos);
    if(tlen < 0) return tlen; else pos += tlen;
...
int uo_upcall_request::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
    ...
    for (int a0 = 0; a0 < this->num_of_platoon; a0++) {
        enc_size += this->platoon[a0].size() + 4 + 1;
    }
    enc_size += __double_encoded_array_size(NULL, 1);
    enc_size += this->key_point._getEncodedSizeNoHash();
...
uint64_t uo_upcall_request::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == uo_upcall_request::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)uo_upcall_request::getHash };

    uint64_t hash = 0x5280ec74851fc6c4LL +
