...
设置起止点
************************************/
int QuarticSpline::set_points( double x0, double y0, double x1, double y1 )
{
    m_x_start_ = x0;
    m_y_start_ = y0;
    m_x_end_ = x1;
...
deriv[2][3] : { 起点：一阶导，二阶导，三阶导;
                终点：一阶导，二阶导，三阶导 }
***************************************************/
int QuarticSpline::set_boundary( double dy0, double ddy0, double dy1, int flag )
{
    if ( flag == 0 )
    {
...
计算四次样条曲线系数
************************************/
int QuarticSpline::compute_coef( int flag )
{
    ...
    if ( flag == 0 )
    {
        m_c_ = 0.5 * m_second_deriv_start_;
        m_d_ = m_first_deriv_start_;
        m_e_ = m_y_start_;

        m_a_ = ( 0.5*m_second_deriv_start_*delta_x2 + (2.0*m_first_deriv_start_+m_first_deriv_end_)*delta_x - 3.0*delta_y ) / delta_x4;

        m_b_ = ( 4.0*delta_y - m_second_deriv_start_*delta_x2 - (3.0*m_first_deriv_start_+m_first_deriv_end_)*delta_x ) / delta_x3;
...
x_start < x < x_end
************************************/
double QuarticSpline::operator() ( double x ) const
{
    ...
    y = m_a_ * pow( dx, 4 ) + m_b_ * pow( dx, 3 ) + m_c_ * dx * dx + m_d_ * dx + m_e_;
...
x_start < x < x_end
************************************/
double QuarticSpline::compute_first_deriv( double x )
{
    assert( x >= m_x_start_ && x <= m_x_end_ );
    ...
    first_deriv_y = 4.0 * m_a_ * pow( dx, 3 ) + 3.0 * m_b_ * dx * dx + 2.0 * m_c_ * dx + m_d_;
...
x_start < x < x_end
************************************/
double QuarticSpline::compute_second_deriv( double x )
{
    assert( x >= m_x_start_ && x <= m_x_end_ );
    ...
    second_deriv_y = 12.0 * m_a_ * dx * dx + 6.0 * m_b_ * dx + 2.0 * m_c_;
...
拟合后计算任意点的头指向
************************************/
double QuarticSpline::compute_heading( double x )
{
    assert( x >= m_x_start_ && x <= m_x_end_ );
...
拟合后计算任意点的曲率
************************************/
double QuarticSpline::compute_k( double x )
{
    assert( x >= m_x_start_ && x <= m_x_end_ );
...
提取系数
************************************/
int QuarticSpline::get_coef( double *coef )
{
    coef[ 0 ] = m_a_;
    coef[ 1 ] = m_b_;
    coef[ 2 ] = m_c_;
...
生成曲线
************************************/
int QuarticSpline::gen_trj_from_two_points( navi_point pos0, navi_point pos1, double step )
{
    ...
    set_boundary( d_start, dd_start, d_end, 0 );                ///设置边界条件
    compute_coef( 0 );
...
int QuarticSpline::get_path_local( vector< navi_point > &out_path_local )
{
...
int QuarticSpline::get_path_global( vector< navi_point > &out_path_global )
{
