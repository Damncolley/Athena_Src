...
double getClosestPointOnSpline(tk::spline& sp_x,
                               tk::spline& sp_y,
                               ...
                               int maxIterations = 20)
{
    ...
    for (int i=0; i<maxIterations; i++)
    {
        ...
        p0.position_x = sp_x(s[0]); p0.position_y = sp_y(s[0]);
        p1.position_x = sp_x(s[1]); p1.position_y = sp_y(s[1]);
        p2.position_x = sp_x(s[2]); p2.position_y = sp_y(s[2]);

        Ds[0] = length_of_two_navipoint(p0, testPoint);  // 返回这个向量的长度的平方（只读）。
        Ds[1] = length_of_two_navipoint(p1, testPoint);
        Ds[2] = length_of_two_navipoint(p2, testPoint);
        ...
        pt1.position_x = sp_x(sk - width); pt1.position_y = sp_y(sk - width);
        pt2.position_x = sp_x(sk);         pt2.position_y = sp_y(sk);
        pt3.position_x = sp_x(sk + width); pt3.position_y = sp_y(sk + width);

        double Ds_pt1 = length_of_two_navipoint(pt1, testPoint);
        double Ds_pt2 = length_of_two_navipoint(pt2, testPoint);
        double Ds_pt3 = length_of_two_navipoint(pt3, testPoint);
