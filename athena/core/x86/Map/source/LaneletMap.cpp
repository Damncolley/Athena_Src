...
LaneletMap::LaneletMap(std::string filename) : _lanelets(LLet::parse_xml(filename)),_regulatory_element(LLet::parse_regulatory_element(filename))
{
    init();
}
...
std::vector<lanelet_ptr_t> LaneletMap::shortest_path(const lanelet_ptr_t &start, const lanelet_ptr_t &dest) const
{
    ...
    std::transform(sp->cbegin(), sp->cend(), sp_ll.begin(), [this](int64_t index){ return graph()[index].lanelet;});
...
void LaneletMap::init()
{
    ...
    for( const auto& src: _lanelets )
    {
        ...
        {
            if(src->fits_before(other))//与之前节点相连
            {
                ...
                //std::cout<<"index_src:"<<index_src<<"    index_dest:"<<index_dest;
                assert(inserted);
                EdgeInfo info;
                info.src=src->id();//yu
                info.dest=other->id();
            ...
            if(src->fits_right(other))//与右边车道边界相连
            {
                int64_t index_dest = this->vertex_id_by_lanelet( other );
                bool inserted;
                Graph::edge_descriptor new_edge;
                ...
                info.src=src->id();//yu
                info.dest=other->id();
                info.turn_right = 500.0;
                info.routing_cost=info.turn_right;
                _graph[new_edge] = info;
            }

            if(src->fits_left(other))//与右边车道边界相连
            {
                int64_t index_dest = this->vertex_id_by_lanelet( other );
                bool inserted;
                Graph::edge_descriptor new_edge;
                ...
                info.src=src->id();//yu
                info.dest=other->id();
                info.turn_left = 500.0;
                info.routing_cost=info.turn_left;
                _graph[new_edge] = info;
            }
        }
    }
