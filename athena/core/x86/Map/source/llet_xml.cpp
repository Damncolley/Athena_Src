...
struct XMLParser
{
    ...
    std::vector< regulatory_element_ptr_t > parsed_regulatory_element() const;
...
std::vector< regulatory_element_ptr_t > XMLParser::parsed_regulatory_element() const
{
    std::vector< regulatory_element_ptr_t > result;
    for( auto& kv : regulatory_elements_by_id )
    {
        result.push_back( kv.second );
    }
...
void XMLParser::parse_nodes()
{
    for ( pugi::xpath_node node : doc.select_nodes("//node[@lat and @lon and @id]") )
    {
        ...
        double id = node.node().attribute("id").as_double();//double//yu//3.22
...
void XMLParser::parse_linestrips()
{
    for( pugi::xpath_node way : doc.select_nodes("//way[@id]") )
    {
        ...
        linestrips_by_id[way.node().attribute("id").as_double()] = new_linestrip;//double//yu//3.22
    }
}
...
void XMLParser::parse_lanelets()
{
    for( pugi::xpath_node relation : doc.select_nodes("//relation/tag[@v='lanelet' and @k='type']/..") )
    {
        ...
        for(std::string role : {"left", "right"})
        {
            ...
            {
                int64_t id = member.node().attribute("ref").as_double();//double//yu//3.22
        ...
        int64_t id = relation.node().attribute("id").as_double();//double//yu//3.22
        ...
        if( ! inrange(phi(),  0.2 * pi_2, 2.0 * pi_2 ) )
        {
            bounds["left"] = std::make_shared< ReversedLineStrip >(bounds["left"]);
            bounds["right"] = std::make_shared< ReversedLineStrip >(bounds["right"]);
        }

        assert( inrange(phi(),  0.2 * pi_2, 2.0 * pi_2 ) );
...
void XMLParser::parse_regulatory_elements_and_assign_to_lanelets()
{
    for( pugi::xpath_node relation : doc.select_nodes("//relation/tag[@v='regulatory_element' and @k='type']/..") )
    {
        ...
        {
            int64_t ref = member.node().attribute("ref").as_double();//double//yu//3.22
    ...
    {
        int64_t relation_id = lanelet.node().attribute("id").as_double();//double//yu//3.22
        for( pugi::xpath_node reg_elem_member : lanelet.node().select_nodes("member[@type='relation' and @role='regulatory_element']") )
        {
            int64_t ref = reg_elem_member.node().attribute("ref").as_double();//double//yu//3.22
...
std::vector< lanelet_ptr_t > LLet::parse_xml(std::string filename)
{
    ...
    //写入
    //pugi::xml_node node = doc_w.append_child("node");
    ...
    //doc.save_file("/home/nextcar/Desktop/result.osm");

    return parser.parsed_lanelets();
}


std::vector< regulatory_element_ptr_t > LLet::parse_regulatory_element(std::string filename)
{
    pugi::xml_document doc;
    /*pugi::xml_parse_result res = */doc.load_file(filename.c_str());
    ...
    return parser.parsed_regulatory_element();
}
